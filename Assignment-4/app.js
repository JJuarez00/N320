...
class MusicPlayer {
    ...
    loadTracks(trackDataArray) {
        trackDataArray.forEach(data => {
            const track = new Track(data.title, data.artist, data.genre, data.src);
...
//
//  Then:                                                               <- Note: Cannot be tested until Part 4 is done.
...
class Playlist {
    ...
    removeTrack(title) {
        const filtered = this.tracks.filter(track => track.title.toLowerCase() !== title.toLowerCase());
...
const playlists = []; // Moved this up here to fix playlist not rendering after creation.
...
document.getElementById('clearLogBtn').addEventListener('click', () => {
    ...
    container.innerHTML = '<div class="log-placeholder">Activity log cleared.</div>';
});
...
// Create "My Favourites" playlist and add first 3 tracks
